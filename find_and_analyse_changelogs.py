    ...
    for i, line in enumerate(line_list):
        ...
        if line.startswith("----") or i+1 == len(line_list): # marks start+end of blocks
            if active_block:
                ...
                lines_in_block=[]
                for block_i, lines in enumerate(line_list):  # should make this a function.
                    if start<block_i<i:
                        if lines == '':
                            continue
                        if not (line.startswith("----") or i+1 == len(line_list)): # makes sure it appends the last entry
                            continue
                        lines_in_block.append(lines) # not sure if I want to do this and then finesse the results separately or just do it directly. Really should do it directly if there's no benefit to double handling.
